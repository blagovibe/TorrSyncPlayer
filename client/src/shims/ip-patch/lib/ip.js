...
ip.isPrivate = function (addr) {
  ...
  // check private ranges with strict validation (each octet 0-255)
  return /^(::f{4}:)?10\.(25[0-5]|2[0-4]\d|1\d\d|[1-9]?\d)\.(25[0-5]|2[0-4]\d|1\d\d|[1-9]?\d)\.(25[0-5]|2[0-4]\d|1\d\d|[1-9]?\d)$/i.test(addr)
    || /^(::f{4}:)?192\.168\.(25[0-5]|2[0-4]\d|1\d\d|[1-9]?\d)\.(25[0-5]|2[0-4]\d|1\d\d|[1-9]?\d)$/i.test(addr)
    || /^(::f{4}:)?172\.(1[6-9]|2\d|30|31)\.(25[0-5]|2[0-4]\d|1\d\d|[1-9]?\d)\.(25[0-5]|2[0-4]\d|1\d\d|[1-9]?\d)$/i
      .test(addr)
    || /^(::f{4}:)?169\.254\.(25[0-5]|2[0-4]\d|1\d\d|[1-9]?\d)\.(25[0-5]|2[0-4]\d|1\d\d|[1-9]?\d)$/i.test(addr)
...
ip.isLoopback = function (addr) {
  // Normalize the address to canonical form before checking
  // This handles edge cases like 127.1, 01200034567, 012.1.2.3, 0x7f.0.0.1
  if (!/:/.test(addr)) {
    // For IPv4, normalize through toLong/fromLong to get canonical form
    // Check if it looks like dotted decimal or a single number
    if (/^\d+(\.\d+)*$/.test(addr) || /^-?\d+$/.test(addr)) {
      const ipl = ip.normalizeToLong(addr);
      if (ipl >= 0) {
        addr = ip.fromLong(ipl);
      } else if (/^-?\d+$/.test(addr)) {
