...
protected:
    void SetUp() override {
        ...
        m_mockTorrentManager = new MockTorrentManager(this);
    }
...
{
    EXPECT_CALL(*m_mockTorrentManager, addTorrent("magnet:?xt=urn:btih:test123"));
    m_mockTorrentManager->addTorrent("magnet:?xt=urn:btih:test123");
}
...
{
    EXPECT_CALL(*m_mockTorrentManager, removeTorrent("test-id"));
    m_mockTorrentManager->removeTorrent("test-id");
}
...
{
    EXPECT_CALL(*m_mockTorrentManager, getFiles("torrent-123"));
    m_mockTorrentManager->getFiles("torrent-123");
}

TEST_F(TorrentManagerGMockTest, SelectFile)
{
    EXPECT_CALL(*m_mockTorrentManager, selectFile("torrent-123", 0));
    m_mockTorrentManager->selectFile("torrent-123", 0);
}
...
TEST_F(TorrentManagerGMockTest, CurrentTorrentId)
{
    ...
    EXPECT_EQ(m_mockTorrentManager->currentTorrentId(), "test-id");
}

TEST_F(TorrentManagerGMockTest, SetCurrentTorrentId)
{
    EXPECT_CALL(*m_mockTorrentManager, setCurrentTorrentId("new-id"));
    m_mockTorrentManager->setCurrentTorrentId("new-id");
}
...
{
    EXPECT_CALL(*m_mockTorrentManager, streamUrl("torrent-123"))
        .WillOnce(Return(QString("http://localhost:8889/api/v1/torrents/torrent-123/stream")));
    EXPECT_EQ(m_mockTorrentManager->streamUrl("torrent-123"), 
              "http://localhost:8889/api/v1/torrents/torrent-123/stream");
}
...
TEST_F(TorrentManagerGMockTest, TorrentAddedSignal)
{
    ...
    EXPECT_EQ(spy.takeFirst().at(0).toObject()["id"].toString(), "torrent-1");
}
...
TEST_F(TorrentManagerGMockTest, TorrentRemovedSignal)
{
    ...
    emit m_mockTorrentManager->torrentRemoved("torrent-1");
    
    EXPECT_EQ(spy.count(), 1);
    EXPECT_EQ(spy.takeFirst().at(0).toString(), "torrent-1");
}
...
TEST_F(TorrentManagerGMockTest, FilesReceivedSignal)
{
    ...
    emit m_mockTorrentManager->filesReceived("torrent-1", files);
    
    EXPECT_EQ(spy.count(), 1);
    auto args = spy.takeFirst();
    EXPECT_EQ(args.at(0).toString(), "torrent-1");
...
TEST_F(TorrentManagerGMockTest, FileSelectedSignal)
{
    ...
    emit m_mockTorrentManager->fileSelected("torrent-1", 0, 
        "http://localhost:8889/api/v1/torrents/torrent-1/stream");
...
TEST_F(TorrentManagerGMockTest, ErrorSignal)
{
    ...
    emit m_mockTorrentManager->error("Test error message");
...
TEST_F(TorrentManagerGMockTest, OnFilesReceived)
{
    ...
    EXPECT_CALL(*m_mockTorrentManager, onFilesReceived("torrent-1", files));
    m_mockTorrentManager->onFilesReceived("torrent-1", files);
}
...
TEST_F(TorrentManagerGMockTest, VerifyCallOrder)
{
    ...
    EXPECT_CALL(*m_mockTorrentManager, addTorrent("magnet:?xt=urn:btih:test1"));
    EXPECT_CALL(*m_mockTorrentManager, listTorrents());
    EXPECT_CALL(*m_mockTorrentManager, getFiles("test-id"));
    EXPECT_CALL(*m_mockTorrentManager, selectFile("test-id", 0));
    EXPECT_CALL(*m_mockTorrentManager, removeTorrent("test-id"));
    
    m_mockTorrentManager->addTorrent("magnet:?xt=urn:btih:test1");
    m_mockTorrentManager->listTorrents();
    m_mockTorrentManager->getFiles("test-id");
    m_mockTorrentManager->selectFile("test-id", 0);
    m_mockTorrentManager->removeTorrent("test-id");
}
...
TEST_F(TorrentManagerGMockTest, MultipleCalls)
{
    ...
    m_mockTorrentManager->addTorrent("magnet:?xt=urn:btih:test1");
    m_mockTorrentManager->addTorrent("magnet:?xt=urn:btih:test2");
    m_mockTorrentManager->addTorrent("magnet:?xt=urn:btih:test3");
}
...
{
    EXPECT_CALL(*m_mockTorrentManager, addTorrent(""));
    m_mockTorrentManager->addTorrent("");
}

TEST_F(TorrentManagerGMockTest, SelectFileWithInvalidIndex)
{
    EXPECT_CALL(*m_mockTorrentManager, selectFile("torrent-1", -1));
    m_mockTorrentManager->selectFile("torrent-1", -1);
}

TEST_F(TorrentManagerGMockTest, SelectFileWithLargeIndex)
{
    EXPECT_CALL(*m_mockTorrentManager, selectFile("torrent-1", 999));
    m_mockTorrentManager->selectFile("torrent-1", 999);
}
